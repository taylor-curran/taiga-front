...
- Name: migration-audit.spec.ts >> migration audit — Angular is source of truth >> project home: section Team heading (requires auth in Angular)
- Location: specs/migration-audit.spec.ts:133:3
...
Locator: locator('h2.title').filter({ hasText: /^Team$/ }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h2.title').filter({ hasText: /^Team$/ }).first()
