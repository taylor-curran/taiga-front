...
- Name: migration-audit.spec.ts >> migration audit — Angular is source of truth >> kanban: main section h1 is Kanban (requires auth in Angular)
- Location: specs/migration-audit.spec.ts:223:3
...
Locator: getByRole('heading', { name: 'Kanban' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Kanban' }).first()
