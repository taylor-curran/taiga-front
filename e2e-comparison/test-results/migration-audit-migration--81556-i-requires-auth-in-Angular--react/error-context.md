...
- Name: migration-audit.spec.ts >> migration audit — Angular is source of truth >> wiki: main section h1 is Wiki (requires auth in Angular)
- Location: specs/migration-audit.spec.ts:241:3
...
Locator: getByRole('heading', { name: 'Wiki' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Wiki' }).first()
