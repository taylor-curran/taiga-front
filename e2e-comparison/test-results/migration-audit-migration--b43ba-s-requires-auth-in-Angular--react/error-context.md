...
- Name: migration-audit.spec.ts >> migration audit — Angular is source of truth >> issues: main section h1 is Issues (requires auth in Angular)
- Location: specs/migration-audit.spec.ts:232:3
...
Locator: getByRole('heading', { name: 'Issues' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Issues' }).first()
