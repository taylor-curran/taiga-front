 ...
 * API URL endpoints.
 *
 * Mirrors the URL map defined in `app/coffee/modules/resources.coffee`.
 *
 * Templated URLs use `%s` placeholders, matching the convention used by the
 * AngularJS `$tgUrls` service. Use `resolveUrl(name, ...args)` to substitute.
 */
export const urls = {
  // Auth
  "auth": "/auth",
...
/** Substitute `%s` placeholders in a URL template with the provided arguments. */
export function resolveUrl(name: UrlName, ...args: Array<string | number>): string {
  let url: string = urls[name];
  for (const arg of args) {
    url = url.replace("%s", String(arg));
  }
  return url;
}
