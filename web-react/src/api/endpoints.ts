...
export function resolveUrl(name: string, ...args: (string | number)[]): string {
  const template = API_URLS[name];
  if (!template) {
    throw new Error(`Unknown API URL key: ${name}`);
  }
  let resolved = template;
  for (const arg of args) {
    resolved = resolved.replace("%s", String(arg));
  }
  return API_PREFIX + resolved;
}
