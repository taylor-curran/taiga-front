...
function orderParam(period: string): string {
  if (period === 'all') return '-total_fans';
  return `-total_fans_last_${period}`;
}
...
export function MostLiked() {
  ...
  const q = useQuery({
    queryKey: ['discover-most-liked', period],
    queryFn: async () => {
    ...
    <section className="discover-highlight">
      ...
      {q.isPending ? (
        ...
        <div className="discover-highlight-grid">
          {(q.data ?? []).length === 0 ? (
            <p>{t('DISCOVER.MOST_LIKED_EMPTY')}</p>
          ) : (
