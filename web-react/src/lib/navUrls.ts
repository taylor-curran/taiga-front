/**
 * Port of `app/coffee/modules/base.coffee` navigation URL map (name → path template).
 * `resolve` strips a leading slash to match `$tgNavUrls.resolve` behaviour.
 */
...
export function formatNavUrl(url: string, ctx: Record<string, string | number | undefined> = {}): string {
...
export function resolveNavUrl(name: keyof typeof NAV_URLS | string, ctx?: Record<string, string | number | undefined>): string {
  const template = NAV_URLS[name as string];
  if (!template) return '';
  const withCtx = ctx ? formatNavUrl(template, ctx) : template;
  return withCtx.startsWith('/') ? withCtx.slice(1) : withCtx;
}
