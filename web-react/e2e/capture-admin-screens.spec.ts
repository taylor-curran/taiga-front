...
test.describe('Reference vs React screenshots (optional when both servers run)', () => {
  ...
  test('build composite diff for login (auth slice)', async ({ browser }) => {
    await mkdir(artifactDir, { recursive: true });
    const refPng = path.join(artifactDir, 'login-ref.png');
    const reactPng = path.join(artifactDir, 'login-react.png');
    ...
    const [refRes, reactRes] = await Promise.all([
      pRef.goto(loginPath, { waitUntil: 'domcontentloaded', timeout: 10_000 }),
      pReact.goto(loginPath, { waitUntil: 'domcontentloaded', timeout: 10_000 }),
    ]);

    test.skip(
      !refRes?.ok() || !reactRes?.ok(),
      'Start Angular :9000 and Vite :5173 for login compare art.',
    );
    ...
    await assertMeaningfulImage(refPng);
    await assertMeaningfulImage(reactPng);
    const out = await compareScreenshotsPng(refPng, reactPng, path.join(artifactDir, 'login-compare'));
    await copyFile(out.compositePath, path.join(artifactDir, 'login-side-by-side.png'));
  });
});
